...
# Compact remote-deploy helpers for Agora (guestkit-style, no cross-repo imports).
...
agora_info()  { printf '  %sOK%s  %s\n' "$_C_GREEN" "$_C_RESET" "$*"; }
agora_warn()  { printf '  %sWARN%s %s\n' "$_C_YELLOW" "$_C_RESET" "$*"; }
agora_error() { printf '  %sERR%s  %s\n' "$_C_RED" "$_C_RESET" "$*"; exit 1; }
agora_step()  { printf '\n  %s==>%s %s%s%s\n' "$_C_CYAN" "$_C_RESET" "$_C_BOLD" "$*" "$_C_RESET"; }
agora_kv()    { printf '  %s%-12s%s %s\n' "$_C_DIM" "$1" "$_C_RESET" "$2"; }
