...
export function WarRoomRail() {
  const channel = useRelay((s) => s.channels.find((c) => c.id === s.activeChannelId));
  const wave = useRelay((s) => s.waves.find((w) => w.id === channel?.waveId));
  const toggleCheck = useRelay((s) => s.toggleCheck);
  ...
  return (
    <aside className="hidden h-full min-h-0 w-72 shrink-0 flex-col border-l border-border bg-sidebar lg:flex">
      ...
      <div className="relay-scroll min-h-0 flex-1 px-3 py-3">
