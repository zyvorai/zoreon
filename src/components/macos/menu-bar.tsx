...
const menus = ["Relay", "File", "Edit", "View", "Go", "Window"] as const;

export function MenuBar() {
  const [clock, setClock] = useState("");
  const [open, setOpen] = useState<string | null>(null);
  const setPaletteOpen = useRelay((s) => s.setPaletteOpen);
  ...
  return (
    <header className="relative z-40 flex h-7 shrink-0 items-center justify-between bg-menubar px-3 text-xs text-fg backdrop-blur-xl">
      <nav className="flex items-center gap-0.5">
        ...
        {menus.map((item) => (
          <div key={item} className={cn("relative", item !== "Relay" && "hidden sm:block")}>
            <button
              type="button"
              onClick={() => setOpen(open === item ? null : item)}
              className={cn(
                "rounded-xs px-2 py-0.5 font-medium",
                item === "Relay" && "font-semibold",
