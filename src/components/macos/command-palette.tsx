...
export function CommandPalette() {
  const open = useRelay((s) => s.paletteOpen);
  const setOpen = useRelay((s) => s.setPaletteOpen);
  const channels = useRelay((s) => s.channels);
  const setActiveChannel = useRelay((s) => s.setActiveChannel);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-desktop/50 px-4 pt-[12vh] backdrop-blur-sm">
      ...
      >
        ...
        <Command.List className="relay-scroll max-h-80 p-1">
